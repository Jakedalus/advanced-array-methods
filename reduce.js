...
function vowelCount(str){
   return str.split("").reduce(function(acc, next) {
       if ('aeiou'.indexOf(next.toLowerCase()) !== -1) {
           if(acc.hasOwnProperty(next)) {
               acc[next]++;
           } else {
               acc[next] = 1;
...
function addKeyAndValue(arr, key, value){
    return arr.reduce(function(acc, next){
        console.log(JSON.stringify(acc), JSON.stringify(next));
        next[key] = value;
        acc.push(next);
        return acc;
    }, []);
...
console.log("addKeyAndValue:", addKeyAndValue(arr, 'title', 'Instructor'));
